...
def rapidlyExploringRandomTree(img, start, goal, seed=None):
    ...
    minX = max(goal[0] - 10 * STEP_DISTANCE, 0)  # 增大搜索范围
    maxX = min(goal[0] + 10 * STEP_DISTANCE, len(img[0]) - 1)
    minY = max(goal[1] - 10 * STEP_DISTANCE, 0)
    maxY = min(goal[1] + 10 * STEP_DISTANCE, len(img) - 1)

    minX = min(minX, maxX)  # 确保 minX 不大于 maxX
    minY = min(minY, maxY)  # 确保 minY 不大于 maxY

    print(f"minX: {minX}, maxX: {maxX}, minY: {minY}, maxY: {maxY}")  # 添加调试信息
...
def find_path_RRT(start, goal, my_map):
    gray_map = map_img(my_map)
    gray_map = cv2.cvtColor(gray_map, cv2.COLOR_GRAY2BGR)[::-1]
    path, graph = rapidlyExploringRandomTree(gray_map, start, goal)
    return path, graph
...
def map_img(arr):
    ...
    for i in range(height):
        for j in range(width):
            if arr[i, j] == -1:
                img[i, j] = 127  # 灰色表示未知区域
            elif arr[i, j] == 0:
                img[i, j] = 255  # 白色表示自由空间
            elif arr[i, j] == 100:
                img[i, j] = 0  # 黑色表示障碍物
            else:
                val = int((1 - arr[i, j] / 100.0) * 255)
                img[i, j] = val
